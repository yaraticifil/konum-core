...
class DriverController extends GetxController {
  ...
  Future<void> startRide() async {
    ...
    try {
      ...
      await _errorLogger.log(ErrorModel(code: "RIDE_START_FAIL", message: e.toString(), source: "DriverController.startRide", occurredAt: DateTime.now()));
    }
  }
  ...
  Future<void> completeRide() async {
    ...
    try {
      ...
      AppNotifier.snackbar("Tamamlandı", "Yolculuk başarıyla tamamlandı.");
    } catch (e) {
      debugPrint("Tamamlama hatası: $e");
      await _errorLogger.log(ErrorModel(code: "RIDE_COMPLETE_FAIL", message: e.toString(), source: "DriverController.completeRide", occurredAt: DateTime.now()));
    }
  }
  ...
  Future<void> updateIban(String newIban) async {
    ...
    try {
      ...
      await fetchDriverData(driver.value!.id);
      AppNotifier.snackbar("Başarılı", "IBAN numaranız kaydedildi.");
    } catch (e) {
  ...
  Future<void> reportPenalty({
    ...
    try {
      ...
      await storageRef.putData(byteData, SettableMetadata(contentType: 'image/${p.extension(image.path).replaceAll('.', '')}'));
      ...
      AppNotifier.snackbar("Başarılı", "Ceza bildirimi iletildi.");
    } catch (e) {
  ...
  Future<void> fetchPayouts([String? driverId]) async {
    ...
    try {
      final snap = await _firestore.collection('payouts').where('driverId', isEqualTo: id).get();
  ...
  double getTotalEarnings() => payouts.where((p) => p.status == PayoutStatus.completed).fold(0.0, (t, i) => t + i.amount);
  double getPendingPayouts() => payouts.where((p) => p.status == PayoutStatus.pending).fold(0.0, (t, i) => t + i.amount);
  ...
  Future<void> toggleOnlineStatus() async {
    if (isOnline.value) { await goOffline(); } else { await goOnline(); }
  }
}
